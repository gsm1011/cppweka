 ...
 */
class FindWithCapabilities : public OptionHandler,
			     public CapabilitiesHandler, 
			     public RevisionHandler {
protected:
  /** the capabilities to look for. */
  Capabilities m_Capabilities = new Capabilities(this);

  /** the capabilities to look for to "not have". */
  Capabilities m_NotCapabilities = new Capabilities(this);

  /** the packages to search in. */
  Vector m_Packages = new Vector();
  ...
  SingleIndex m_ClassIndex = new SingleIndex();
...
public:
  ...
  void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0)
      ...
      throw IllegalArgumentException("A superclass has to be specified!");
   ...
   * @return		true if the capability is enabled
   */
  bool isEnabledNot(Capability c) {
