 ...
 * @see Loader
 */
class C45Loader : public AbstractFileLoader, public BatchConverter, public IncrementalConverter {
  /** for serialization */
  // static final long serialVersionUID = 5454329403218219L;
  
public: 
  /** the file extension */
  static string FILE_EXTENSION = ".names";

private:
  /**
   * Describe variable <code>m_sourceFileData</code> here.
   */
  File m_sourceFileData = NULL;

  /**
   * Reader for names file
   */
  transient Reader m_namesReader = NULL;

  /**
   * Reader for data file
   */
  transient Reader m_dataReader = NULL;
...
private:
  ...
  void readHeader(StreamTokenizer tokenizer) throws IOException {
    ...
    m_ignore = new bool[m_numAttribs];
...
void main (int argc, char *argv[]) {
  runFileLoader(new C45Loader(), args[0]);
}
