 ...
 * @see Loader
 */
class DatabaseLoader : public AbstractLoader, 
			      public BatchConverter, 
			      public IncrementalConverter,
...
private: 
  ...
  string m_query = "Select * from Results0";
...
public: 
   ...
   */
  DatabaseLoader() throws Exception{
   ...
   * @param password the password
   */  
  void setSource(string url, string userName, string password){
...
private:
   ...
   */  
  string endOfQuery(bool onlyTableName){
      string table;
   ...
   */  
  bool checkForKey() throws Exception {
   ...
   */  
  void stringToNominal(ResultSet rs, int i) throws Exception{
   ...
   */  
  string limitQuery(string query, int offset, int choice){
      ...
      if(m_checkForTable) {
        ...
        while (!rightChoice){
            try{
                ...
                choice++;
                if(choice == 3){
                    cout << "Incremental loading not supported for "
			 << "that DBMS. Pseudoincremental mode is used "
			 << "if you use incremental "
			 << "loading.\nAll rows are loaded into memory "
			 << "once and retrieved "
			 << "incrementally from memory instead of from the database." << endl;
        ...
        for (int i = 1; i <= numAttributes; i++) {
            switch (m_DataBaseConnection.translateDBColumnType(md.getColumnTypeName(i))) {
                ...
                case DatabaseConnection.TEXT:
                    //System.err.println("bool --> string");
                ...
                case DatabaseConnection.BOOL:
                    //System.err.println("bool --> nominal");
                    attributeTypes[i - 1] = Attribute.NOMINAL;
                    m_nominalIndexes[i - 1] = new Hashtable();
        ...
        //cout << m_structure.attribute(0).name().equals(idColumn) << endl;
...
private:
   ...
   */  
  Instance readInstance(ResultSet rs) throws Exception{
...
private: 
  ...
  void printException(Exception ex) {
	...
	cout << "SQLState:  " << ((SQLException)ex).getSQLState () << endl;
	cout << "ErrorCode: " << ((SQLException)ex).getErrorCode () << endl;
...
public: 
  string getRevision() {
    return RevisionUtils.extract("$Revision: 6332 $");
  }
}; 
