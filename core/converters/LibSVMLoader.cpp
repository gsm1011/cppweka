 ...
 * @see Loader
 */
class LibSVMLoader : public AbstractFileLoader, public BatchConverter,
...
protected: 
  ...
  Vector m_Buffer = NULL;
  
public: 
  /** the file extension. */
  static string FILE_EXTENSION = ".libsvm";
...
protected: 
   ...
   */
  double[] libsvmToArray(string row) {
    double[]		result;
    StringTokenizer	tok;
    int			index;
    int			max;
   ...
   */
  int determineNumAttributes(string row, int num) {
    int		result;
    int		count;
...
void main(int argc, char* argv[]) {
  runFileLoader(new LibSVMLoader(), argv[0]);
}
