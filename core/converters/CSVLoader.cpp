 ...
 * @see Loader
 */
class CSVLoader : public AbstractFileLoader, public BatchConverter, public OptionHandler {
...
protected: 
  ...
  transient BufferedReader m_sourceReader;
  
  /** Tokenizer for the data. */
  transient StreamTokenizer m_st;
  
  /** The range of attributes to force to type nominal. */
  Range m_NominalAttributes = new Range();
  
  /** The range of attributes to force to type string. */
  Range m_StringAttributes = new Range();
...
public: 
   ...
   *         		displaying in the explorer/experimenter gui
   */
  string nominalAttributesTipText() {
    return 
   ...
   *         		displaying in the explorer/experimenter gui
   */
  string stringAttributesTipText() {
    return 
...
public: 
   ...
   */
private:
  FastVector getInstance(StreamTokenizer tokenizer) 
    throws IOException {
   ...
   * </jml></pre>
   */
  void checkStructure(FastVector current) throws Exception {
    if (current == NULL) {
   ...
   * </jml></pre>
   */
  void readHeader(StreamTokenizer tokenizer) throws IOException {
   ...
   */
  void initTokenizer(StreamTokenizer tokenizer) {
...
void main(int argc, char *argv[]) {
  runFileLoader(new CSVLoader(), argv[0]);
}
