 ...
 * @see Loader
 */
class SVMLightLoader : public AbstractFileLoader, 
...
public: 
  /** the file extension. */
  static string FILE_EXTENSION = ".dat";
...
protected: 
   ...
   */
  double[] svmlightToArray(string row) throws Exception {
    double[]		result;
    StringTokenizer	tok;
    int			index;
    int			max;
   ...
   */
  int determineNumAttributes(double[] values, int num) throws Exception {
    int		result;
    int		count;
   ...
   */
  Attribute determineClassAttribute() {
    Attribute	result;
    bool	binary;
    int		i;
    FastVector	values;
...
void main( int argc, char* argv[] ) {
  runFileLoader(new SVMLightLoader(), argv[0]);
}
