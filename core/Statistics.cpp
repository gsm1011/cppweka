 ...
 */
class Statistics : public RevisionHandler {
...
public:
  ...
  static double normalInverse(double y0) { 
    ...
    if( y0 <= 0.0 ) throw IllegalArgumentException();
    if( y0 >= 1.0 ) throw IllegalArgumentException();
    code = 1;
    y = y0;
  ...
  static double lnGamma(double x) {
    ...
    if( x < -34.0 ) {
      ...
      w = lnGamma(q);
      p = floor(q);
      if( p == q ) throw ArithmeticException("lnGamma: Overflow");
      z = q - p;
      if( z > 0.5 ) {
	p += 1.0;
	z = p - q;
	...
	x += 1.0;
      ...
      if( z < 0.0 ) z = -z;
      if( x == 2.0 ) return log(z);
      x -= 2.0;
      p = x * polevl( x, B, 5 ) / p1evl( x, C, 6);
      return( log(z) + p );
    }
 
    if( x > 2.556348e305 ) throw ArithmeticException("lnGamma: Overflow");
  ...
  static double gamma(double x) {
    ...
    if( q > 33.0 ) {
      if( x < 0.0 ) {
	p = floor(q);
	if( p == q ) throw ArithmeticException("gamma: overflow");
	i = (int)p;
	z = q - p;
	if( z > 0.5 ) {
	  p += 1.0;
	...
	z = q * sin( PI * z );
	if( z == 0.0 ) throw ArithmeticException("gamma: overflow");
	z = abs(z);
    ...
    while( x < 0.0 ) {
      if( x == 0.0 ) {
	throw ArithmeticException("gamma: singular");
      } else
	if( x > -1.E-9 ) {
  ...
  static double incompleteBeta( double aa, double bb, double xx ) {
    ...
    if( aa <= 0.0 || bb <= 0.0 ) throw 
      ArithmeticException("ibeta: Domain error!");

    if( (xx <= 0.0) || ( xx >= 1.0) ) {
      if( xx == 0.0 ) return 0.0;
      if( xx == 1.0 ) return 1.0;
      throw ArithmeticException("ibeta: Domain error!");
    }
