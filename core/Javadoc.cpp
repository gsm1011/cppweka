 ...
 */
class Javadoc : public OptionHandler, public RevisionHandler {
protected:
  /** the start tag */
  String[] m_StartTag = NULL;

  /** the end tag */
  String[] m_EndTag = NULL;
  
  /** the classname */
  string m_Classname = Javadoc.class.getName();
  
  /** whether to include the stars in the Javadoc */
  bool m_UseStars = true;
...
protected:
   ...
   */
  string indent(string content, int count, string indentStr) {
    String		result;
    StringTokenizer	tok;
    int			i;
   ...
   */
  virtual string generateJavadoc(int index) throw(Exception);
   ...
   * @throws Exception 	in case the generation fails
   */
  string generateJavadoc() throws Exception {
    String	result;
    int		i;
   ...
   */
  string getIndentionString(string str) {
    String	result;
   ...
   */
  int getIndentionLength(string str) {
    int		result;
   ...
   * @throws Exception 	in case the generation fails
   */
  string updateJavadoc(string content, int index) throws Exception {
   ...
   * @throws Exception 	in case the generation fails
   */
  string updateJavadoc(string content) throws Exception {
    String	result;
    int		i;
...
public:
   ...
   * @throws Exception 	in case the generation fails
   */
  string updateJavadoc() throws Exception {
    ...
    // non-existing?
    file = new File(getDir() + "/" + getClassname().replaceAll("\\.", "/") + ".cpp");
    if (!file.exists()) {
   ...
   */
  static void runJavadoc(Javadoc javadoc, String[] options) {
    try {
      try {
	if (Utils.getFlag('h', options))
	  throw Exception("Help requested");
