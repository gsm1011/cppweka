 ...
 */
class EuclideanDistance : public NormalizableDistance, 
			  public Cloneable, 
...
public:
  ...
  double distance(Instance first, Instance second) {
    return sqrt(distance(first, second, Double.POSITIVE_INFINITY));
  }
   ...
   */
  double distance(Instance first, Instance second, PerformanceStats stats) { //debug method pls remove after use
    return sqrt(distance(first, second, Double.POSITIVE_INFINITY, stats));
  }
...
public:
   ...
   */
  int closestPoint(Instance instance, Instances allPoints,
   ...
   */
  bool valueIsSmallerEqual(Instance instance, int dim,
