 ...
 */
class Optimization : public TechnicalInformationHandler, 
...
protected:
  double m_ALF = 1.0e-4;
    ...
    double m_TOLX = 1.0e-6;
   
    double m_STPMX = 100.0;
...
protected:
  static double m_Epsilon, m_Zero; 
...
public:  
  static {
     ...
     */
protected:
  virtual double objectiveFunction(double[] x) throw(Exception) = 0;
     ...
     * @throws Exception if something goes wrong
     */
  virtual double[] evaluateGradient(double[] x) throw(Exception) = 0;
     ...
     * @throws Exception if something goes wrong
     */
    double[] evaluateHessian(double[] x, int index) throws Exception{
	return NULL;
    }
...
public:
     ...
     */
    double[] lnsrch(double[] xold, double[] gradient, 
	...
	if(m_Slope > m_Zero){
		...
		for(int h=0; h<x.length; h++)
		    System.err.println(h+": isFixed="+isFixed[h]+", x="+
				       ...
				       direct[h]);
	    throw Exception("g'*p positive! -- Try to debug from here: line 327.");
	}
	...
	for(i=0;i<len;i++){	    
		...
		temp=abs(direct[i])/max(abs(x[i]),1.0);
	...
	for (k=0;;k++) {
	    ...
	    if(Double.isNaN(m_f))
		throw Exception("Objective function value is NaN!");
		...
		if(newSlope >= m_BETA*m_Slope){ // Beta condition: ensure pos. defnty.	
		    ...
		    double upper = min(alpha,maxalam); 
		    if(m_Debug)
	    ...
	    if(alam>alpha){
		throw Exception("Sth. wrong in lnsrch:"+
	...
	double stpmax = m_STPMX*max(sqrt(sum), l);
	...
	for(int step=0; step < m_MAXITS; step++){
		...
		for(int g=0; g<l; g++){
		    ...
		    double tmp = abs(grad[g])*
			max(abs(direct[g]),1.0)/
			max(abs(m_f),1.0);
		    if(tmp > test) test = tmp;	
		}
		...
		if(finish){
			...
			if(x[index] >= constraints[1][index]) // Upper bound
			    ...
			    throw Exception("x["+index+"] not fixed on the"+
		...
		if(denom<max(m_Zero*sqrt(dxSq)*sqrt(dgSq), m_Zero)){
		    if (m_Debug) 
		...
		if(Double.isNaN(LDIR[m]))
		    throw Exception("L*direct["+m+"] is NaN!"
					+"|-g="+b[m]+"|"+isFixed[m]
					+"|diag="+D[m]);
	    }
     ...
     */     
    static double[] solveTriangle(Matrix t, double[] b, 
     ...
     */    
protected:
  void updateCholeskyFactor(Matrix L, double[] D, 
	...
	if(coeff>0.0){
		...
		if(Double.isNaN(D[j])){
		    throw Exception("d["+j+"] NaN! P="+P[j]+",d="+d+
					",t="+t+",p="+p+",sigma="+sigma+
					",sclar="+coeff);
		}
		...
		if((j<n-1) && 
		   (Double.isNaN(sigma) || Double.isInfinite(sigma)))
		    throw Exception("sigma NaN/Inf! rho="+rho+
				       ",theta="+theta+",P["+j+"]="+
				       P[j]+",p="+p+",d="+d+",t="+t+
				       ",oldsigma="+sigmaOld);
   ...
   */
private:
  class DynamicIntArray : public RevisionHandler {
    ...
    int m_CapacityIncrement = 1;
  
    /** The capacity multiplier. */
    int m_CapacityMultiplier = 2;
