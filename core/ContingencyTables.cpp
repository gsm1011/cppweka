 ...
 */
class ContingencyTables : public RevisionHandler {
...
public:
  static double chiSquared(double [][] matrix, bool yates) {
   ...
   */
  static double chiVal(double [][] matrix, bool useYates) {
   ...
   */
  static bool cochransCriterion(double[][] matrix) {
  ...
  static double CramersV(double [][] matrix) {
    ...
    if ((min == 0) || Utils.eq(n, 0))
      return 0;
    return sqrt(chiVal(matrix, false) / (n * (double)min)); 
  } 
   ...
   * @return the entropy
   */
  static double entropyConditionedOnRows(double[][] train, 
						double[][] test,
						double numClasses) {
	...
	returnValue -= test[i][j] * log(train[i][j] + 1);
	trainSumForRow += train[i][j];
	testSumForRow += test[i][j];
      }
      testSum = testSumForRow;
      returnValue += testSumForRow * log(trainSumForRow + 
					     numClasses);
    }
...
private:
   ...
   * too close to zero 
   */
  static double chiCell(double freq, double expected, 
                                bool yates){
