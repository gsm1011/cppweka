 ...
 */
class SnowballStemmer : public Stemmer, public OptionHandler {
public:
  /** for serialization. */
  // // static final long serialVersionUID = -6111170431963015178L;
  
  /** the package name for snowball. */
  const static string PACKAGE = "org.tartarus.snowball";
  
  /** the package name where the stemmers are located. */
  const static string PACKAGE_EXT = PACKAGE + ".ext";

protected:
  /** the snowball program, all stemmers are derived from. */
  final static string SNOWBALL_PROGRAM = PACKAGE + ".SnowballProgram";
  ...
  static Vector m_Stemmers;

  /** the current stemmer. */
  Object m_Stemmer;

  /** the stem method. */
  transient Method m_StemMethod;

  /** the setCurrent method. */
  transient Method m_SetCurrentMethod;

  /** the getCurrent method. */
  transient Method m_GetCurrentMethod;
...
public: 
   ...
   */
private:
  static string getStemmerName(string classname) {
   ...
   * @see                 #PACKAGE_EXT
   */
  static string getStemmerClassname(string name) {
   ...
   */
  static Enumeration listStemmers() {
    initStemmers();
   ...
   */
private:
  static string getStemmerList(int lineLength, string indention) {
    string        result;
    Enumeration   enm;
    string        name;
    string        line;
