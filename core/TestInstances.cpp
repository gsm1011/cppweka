 ...
 */
class TestInstances : public Cloneable,
...
public:
  ...
  const static int CLASS_IS_LAST = -1;
  
  /** can be used to avoid generating a class attribute
   * @see #setClassIndex(int) */
  const static int NO_CLASS = -2;

  /** the default list of words used in strings */
  const static String[] DEFAULT_WORDS = {"The", "quick", "brown", "fox", "jumps", "over", "the", "lazy", "dog"};

  /** the default word separators used in strings */
  const static string DEFAULT_SEPARATORS = " ";
  
protected:
  /** for generating string attributes/classes */
  String[] m_Words = DEFAULT_WORDS;
  
  /** for generating string attributes/classes */
  string m_WordSeparators = DEFAULT_SEPARATORS;
  
  /** the name of the relation */
  string m_Relation = "Testdata";
  
  /** the seed value */
  int m_Seed = 1;
  
  /** the random number generator */
  Random m_Random = new Random(m_Seed);
  
  /** the number of instances */
  int m_NumInstances = 20;
  
  /** the class type */
  int m_ClassType = Attribute.NOMINAL;
...
public:
  ...
  void setOptions(String[] options) throws Exception {
	...
	throw IllegalArgumentException("Class '" + tmpStr + "' is not a CapabilitiesHandler!");
      }
    }
...
public:
   ...
   */
  void setMultiInstance(bool value) {
...
protected:
  Attribute generateAttribute(int index, int attType,
    ...
    if (index == CLASS_IS_LAST) {
      ...
      switch (attType) {
        ...
        default:
          throw IllegalArgumentException("Attribute type '" + attType + "' unknown!");
      }
    }
   ...
   */
  double generateClassValue(Instances data) throws Exception {
    double result = Double.NaN;
    
    switch (m_ClassType) {
      case Attribute.NUMERIC:
        result = m_Random.nextFloat() * 0.25
            + abs(m_Random.nextInt())
            % max(2, m_NumNominal);
        break;
        
      case Attribute.NOMINAL:
        result = abs(m_Random.nextInt()) % data.numClasses();
        break;
   ...
   * @throws Exception    if something goes wrong
   */
  double generateAttributeValue(Instances data, int index, double classVal) throws Exception {
    double result = Double.NaN;
...
public:
   ...
   * @throws Exception	if something goes wrong
   */
  Instances generate(string namePrefix) throws Exception {
    if (getMultiInstance()) {
   ...
   * @throws Exception	if something goes wrong
   */
void main(int argc, char* argv[]) throw(Exception) {
    TestInstances inst;
