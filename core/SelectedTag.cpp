 ...
 */
class SelectedTag : public RevisionHandler {
...
public:
  ...
  SelectedTag(int tagID, Tag[] tags) {
    ...
    if (ID.size() != tags.length)
      throw IllegalArgumentException("The IDs are not unique!");
    if (IDStr.size() != tags.length)
      throw IllegalArgumentException("The ID strings are not unique!");
    ...
    throw IllegalArgumentException("Selected tag is not valid");
  }
