 ...
 */
class RandomVariates : public Random, 
...
public:
   ...
   */
  RandomVariates(long seed){ super(seed); }
   ...
   */
protected:
  int next(int bits) {return super.next(bits);}
...
public:
  ...
  double nextExponential(){
    return -log(1.0-super.nextDouble());
  }
   ...
   */
  double nextErlang(int a) throws Exception{
    if(a<1)
      throw Exception("Shape parameter of Erlang distribution must be greater than 1!");
   ...
   * Gamma Variates", ACM Trans. on  Software, Vol.14, No.3, pp261-266.
   * <br>
   ...
   * from Gamma and Poisson distributions", ACM Trans. on  Software, 
   ...
   */
  double nextGamma(double a) throws Exception{
    if(a<=0.0)
      throw Exception("Shape parameter of Gamma distribution"+
      "must be greater than 0!");
    else if (a==1.0)
      return nextExponential();
    else if (a<1.0){
      double b=1.0+exp(-1.0)*a, p,x, condition;
      do{
      ...
      double b=a-1.0, D=sqrt(b), D1,x1,x2,xl,f1,f2,x4,x5,xr,f4,f5,
      p1,p2,p3,p4;
      ...
      if(a<=2.0){
        ...
        x1 = x2-D1;
        xl = 1.0-b/x1;
        f1 = exp(b*log(x1/b)+2.0*D1);
      }

      f2=exp(b*log(x2/b)+D1);
      x4 = b+D;
      x5 = x4+D;
      xr = 1.0-b/x5;
      f4 = exp(b*log(x4/b)-D);
      f5 = exp(b*log(x5/b)-2.0*D);
      p1 = 2.0*f4*D;
      p2 = 2.0*f2*D1+p1;
      p3 = f5/xr+p2;
      p4 = -f1/xl+p3;
      ...
      while(log(w) > (b*log(x/b)+b-x) || x < 0.0){
