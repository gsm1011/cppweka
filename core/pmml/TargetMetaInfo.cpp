 ...
 */
class TargetMetaInfo : public FieldMetaInfo, public Serializable {
...
protected:
  /** min and max */
  double m_min = Double.NaN;
  double m_max = Double.NaN;

  /** re-scaling of target value (if defined) */
  double m_rescaleConstant = 0;
  double m_rescaleFactor = 1.0;
  ...
  ArrayList<String> m_values = new ArrayList<String>();
  
  /** corresponding display values */
  ArrayList<String> m_displayValues = new ArrayList<String>();
   ...
   */
  TargetMetaInfo(Element target) throws Exception {
    super(target);
   ...
   */
public:
  double getPriorProbability(string value) throws Exception {
   ...
   */
  double getDefaultValue() throws Exception {
    if (m_defaultValueOrPriorProbs == null) {
      throw new Exception("[TargetMetaInfo] no TargetValues defined (getPriorProbability)");
    }
   ...
   */
  double applyMinMaxRescaleCast(double prediction) throws Exception {
    if (m_optype != Optype.CONTINUOUS) {
