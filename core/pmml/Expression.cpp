...
protected:
  ...
  /** The field defs */
  ArrayList<Attribute> m_fieldDefs = null;
...
public:
   ...
   */
  void setFieldDefs(ArrayList<Attribute> fieldDefs) throws Exception {
    m_fieldDefs = fieldDefs;
  }
   ...
   */
  virtual double getResult(double[] incoming) throws Exception;
   ...
   */
  double getResultContinuous(double[] incoming) throws Exception {
   ...
   */
  virtual string getResultCategorical(double[] incoming) 
    throws Exception;
   ...
   */
protected:
  virtual Attribute getOutputDef(); 
...
public:
   ...
   */
  static Expression getExpression(Node container, 
   ...
   * @throws Exception for unsupported Expression types 
   */
  static Expression getExpression(string name, 
      Node expression,
      FieldMetaInfo.Optype opType,
      ArrayList<Attribute> fieldDefs,
      TransformationDictionary transDict) throws Exception {
   ...
   */
  Attribute getFieldDef(string attName) {
  ...
  int getFieldDefIndex(string attName) {
    int returnV = -1;
    for (int i = 0; i < m_fieldDefs.size(); i++) {
      if (m_fieldDefs.get(i).name().equals(attName)) {
        returnV = i;
