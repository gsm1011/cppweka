 ...
 */
class MiningSchema : public Serializable {
...
protected:
  ...
  TransformationDictionary m_transformationDictionary = null;

  /** target meta info (may be null if not defined) */
  TargetMetaInfo m_targetMetaInfo = null;
  
private:
 void getLocalTransformations(Element model) throws Exception {
...
public:
   ...
   */
  void applyMissingValuesTreatment(double[] values) throws Exception {
   ...
   */
  void applyOutlierTreatment(double[] values) throws Exception {
    for (int i = 0; i < m_miningMeta.size(); i++) {
      MiningFieldMetaInfo mfi = m_miningMeta.get(i);
   ...
   */
  void applyMissingAndOutlierTreatments(double[] values) throws Exception {
    for (int i = 0; i < m_miningMeta.size(); i++) {
      MiningFieldMetaInfo mfi = m_miningMeta.get(i);
      values[i] = mfi.applyMissingValueTreatment(values[i]);
      values[i] = mfi.applyOutlierTreatment(values[i]);
   ...
   */
  void convertNumericAttToNominal(int index, 
  ...
  ArrayList<DerivedFieldMetaInfo> getDerivedFields() {
    return m_derivedMeta;
  }
  
  ArrayList<MiningFieldMetaInfo> getMiningFields() {
    return m_miningMeta;
  }
