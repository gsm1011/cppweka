 ...
 */
class NearestNeighbourSearch : public Serializable, 
			       public OptionHandler,
			       public AdditionalMeasureProducer,
...
protected: 
  ...
  public:   
     ...
     */
    void put(int i, double d) throws Exception {
     ...
     * 				element
     */
    void putBySubstitute(int i, double d) throws Exception {
     ...
     * @param d			the distance
     */
    void putKthNearest(int i,  double d) {
   ...
   */ //better to change this into a heap element
    class NeighborNode : public RevisionHandler {
    protected:
     ...
     */
    NeighborNode(double distance, Instance instance, NeighborNode next) {
      m_Distance = distance;
      m_Instance = instance;
      m_Next = next;
    }
     ...
     * @param instance 		the neighbor instance
     */
    NeighborNode(double distance, Instance instance) {
  ...
  class NeighborList : public RevisionHandler {
   protected: 
    /** The first node in the list. */
    NeighborNode m_First;
  ...
  public:       
    ...
    void printList() {
      ...
      if (isEmpty()) {
        ...
        NeighborNode current = m_First;
        while (current != null) {
          cout << "Node: instance " << current.m_Instance 
	       <<  ", distance " << current.m_Distance << endl;
          current = current.m_Next;
        }
    cout << endl; 
      }
...
public: 
   ...
   *         		displaying in the explorer/experimenter gui
   */
  string measurePerformanceTipText() {
   ...
   */
  abstract Instance nearestNeighbour(Instance target) throws Exception;
   ...
   */
  abstract Instances kNearestNeighbours(Instance target, int k) throws Exception;
   ...
   */
  abstract double[] getDistances() throws Exception;
   ...
   */
  abstract void update(Instance ins) throws Exception;
   ...
   * 				arrayToSort.
   */
  static void combSort11(double arrayToSort[], int linkedArray[]) {
...
protected: static int partition(double[] arrayToSort, double[] linkedArray, int l, int r) {
   ...
   */
public: 
  static void quickSort(double[] arrayToSort, double[] linkedArray, int left, int right) {
    if (left < right) {
