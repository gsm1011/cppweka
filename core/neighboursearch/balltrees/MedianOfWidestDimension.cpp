 ...
 */
class MedianOfWidestDimension : public BallSplitter, 
   ...
   */
protected: bool m_NormalizeDimWidths = true;
...
public:
   ...
   * use for splitting.
   */
  MedianOfWidestDimension(int[] instList, Instances insts, 
                                 EuclideanDistance e) {
   ...
   * given node.
   */
  void splitNode(BallNode node, int numNodesCreated) throws Exception {
...
protected: 
   ...
   */
  int partition(int attIdx, int[] index, int l, int r) {
...
public:  
   ...
   */
  int select(int attIdx, int[] indices, 
   ...
   */
  string normalizeDimWidthsTipText() {
    return 
   ...
   * normalized. 
   */
  void setNormalizeDimWidths(bool normalize) {
