 ...
 */
class MiddleOutConstructor : public BallTreeConstructor
			     public Randomizable,
			     public TechnicalInformationHandler {
...
public:  
   ...
   * the tree.
   */
  BallNode buildTree() throws Exception {
...
protected:
   ...
   */
  BallNode buildTreeMiddleOut(int startIdx, int endIdx) 
    throws Exception {
   ...
   * the hierarchy.
   */
  void createAnchorsHierarchy(Vector anchors, final int numAnchors, 
   ...
   */
  void buildLeavesMiddleOut(BallNode node) throws Exception {
   ...
   * merging.
   */
  BallNode mergeNodes(Vector list, int startIdx, int endIdx)
    throws Exception {
   ...
   */
  BallNode makeBallTreeNodes(TempNode node, int startidx, 
   ...
   */
  TempNode getFurthestFromMeanAnchor(int startIdx, int endIdx) {
   ...
   */
  TempNode getRandomAnchor(int startIdx, int endIdx) {
...
public: 
   ...
   */
  void setPoints(TempNode node, int startIdx, int endIdx, int[] indices) {
   ...
   */
  void setInterAnchorDistances(Vector anchors, TempNode newAnchor,
   ...
   */
  bool stealPoints(TempNode newAnchor, Vector anchors, 
    ...
    bool anyPointsStolen=false, pointsStolen=false;
    TempNode anchorI;
   ...
   */
  Instance calcPivot(TempNode node1, TempNode node2, Instances insts) {
   ...
   */
  Instance calcPivot(MyIdxList list1, MyIdxList list2, Instances insts) {
    int classIdx = m_Instances.classIndex();
    double[] attrVals = new double[insts.numAttributes()];
   ...
   * @throws Exception
   */
  double calcRadius(TempNode n1, TempNode n2) {
   ...
   * @return The radius of the node. 
   */
  double calcRadius(MyIdxList list1, MyIdxList list2, 
   ...
   */
  int[] addInstance(BallNode node, Instance inst) throws Exception {
   ...
   */
  string initialAnchorRandomTipText() {
   ...
   */
  void setInitialAnchorRandom(bool randomInitialAnchor) {
   ...
   * index array. 
   */
  void checkIndicesList(MyIdxList list, int startidx, int endidx) 
    throws Exception {
   ...
   * array. 
   */
  string printInsts(int startIdx, int endIdx) {
   ...
   */
  string printList(MyIdxList points) {
...
protected:
   ...
   * @version $Revision: 1.3 $
   */
  class TempNode : public RevisionHandler {
  public:
   ...
   * @version $Revision: 1.3 $
   */
  class ListNode : public RevisionHandler {
    public:
   ...
   * @version $Revision: 1.3 $
   */
  class MyIdxList : public FastVector {
     ...
     */
    void insertReverseSorted(final int idx, final double distance) {
     ...
     * to list1.
     */
    MyIdxList append(MyIdxList list1, MyIdxList list2) {
     ...
     */
    void checkSorting(MyIdxList list) throws Exception {
