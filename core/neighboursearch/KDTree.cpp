 ...
 */
class KDTree : public NearestNeighbourSearch, public TechnicalInformationHandler {
...
protected: 
  ...
  /** The node splitter. */
  KDTreeNodeSplitter m_Splitter = new SlidingMidPointOfWidestSide();
...
public: 
  ...
  const static int MIN = EuclideanDistance.R_MIN;
  
  /** The index of MAX value in attributes' range array. */
  const static int MAX = EuclideanDistance.R_MAX; 
  
  /** The index of WIDTH (MAX-MIN) value in attributes' range array. */
  const static int WIDTH = EuclideanDistance.R_WIDTH;
...
protected:
   ...
   */
  void buildKDTree(Instances instances) throws Exception {
   ...
   * splitting.
   */
  void splitNodes(KDTreeNode node, double[][] universe,
   ...
   */
  void findNearestNeighbours(Instance target, KDTreeNode node, int k,
    ...
    if (node.isALeaf()) {
      ...
      bool targetInLeft = m_EuclideanDistance.valueIsSmallerEqual(target,
...
public: 
   ...
   */
  void update(Instance instance) throws Exception { // better to change
                                                            // to addInstance
    if (m_Instances == null)
...
protected: 
   ...
   * the instance.
   */
  void addInstanceToTree(Instance inst, KDTreeNode node)
      throws Exception {
    if (node.isALeaf()) {
   ...
   * need to be updated.
   */
  void afterAddInstance(KDTreeNode node) {
    node.m_Start++;
    node.m_End++;
    if (!node.isALeaf()) {
...
protected: 
   ...
   */
  void checkMissing(Instances instances) throws Exception {
   ...
   * instance.
   */
  void checkMissing(Instance ins) throws Exception {
    for (int j = 0; j < ins.numValues(); j++) {
      if (ins.index(j) != ins.classIndex())
        if (ins.isMissingSparse(j)) {
          throw new Exception("ERROR: KDTree can not deal with missing "
   ...
   */
  double getMaxRelativeNodeWidth(double[][] nodeRanges,
      double[][] universe) {
   ...
   * dimension/attribute.
   */
  int widestDim(double[][] nodeRanges, double[][] universe) {
...
protected:
   ...
   */
  void determineAssignments(KDTreeNode node, Instances centers,
   ...
   */
  int[] refineOwners(KDTreeNode node, Instances centers,
   ...
   */
  double distanceToHrect(KDTreeNode node, Instance x) throws Exception {
    double distance = 0.0;
   ...
   */
  bool clipToInsideHrect(KDTreeNode node, Instance x) {
    bool inside = true;
   ...
   */
  bool candidateIsFullOwner(KDTreeNode node, Instance candidate,
    ...
    bool isFullOwner = m_EuclideanDistance.distance(extreme, candidate) < m_EuclideanDistance
...
public: 
   ...
   */
  void assignSubToCenters(KDTreeNode node, Instances centers,
  ...
  bool m_NormalizeNodeWidth = true;
...
public: 
   ...
   * @return 		the tip text for this property
   */
  string normalizeNodeWidthTipText() {
   ...
   */
  void setNormalizeNodeWidth(bool n) {
