 ...
 */
class SpecialFunctions : public RevisionHandler {
private:
  /** Some constants */
...
public: 
  ...
  static double log2Binomial(double a, double b) {
    ...
    if (Utils.gr(b,a)) {
      throw ArithmeticException("Can't compute binomial coefficient.");
    }
...
static void main(String[] ops) {
  ...
  System.out.println("6!: " + exp(SpecialFunctions.lnFactorial(6)));
  System.out.println("Binomial 6 over 2: " +
		     pow(2, SpecialFunctions.log2Binomial(6, 2)));
  System.out.println("Multinomial 6 over 1, 2, 3: " +
		     pow(2, SpecialFunctions.log2Multinomial(6, doubles)));
}
