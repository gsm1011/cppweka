...
public:
   ...
   */
  static string extract(RevisionHandler handler) {
   ...
   */
  static Type getType(RevisionHandler handler) {
   ...
   * @see #extract(String)
   */
  static Type getType(string revision) {
    Type	result;
    String[]	parts;
    int		i;
	...
	if (revision.indexOf('.') == -1)
	  throw Exception("invalid CVS revision - not dots!");
	
	parts = revision.split("\\.");

	// must consist of at least TWO parts/integers
	if (parts.length < 2)
	  throw Exception("invalid CVS revision - not enough parts separated by dots!");
