 ...
 */
class TechnicalInformation : public RevisionHandler {
...
public:
  ...
  protected:
     ...
     */
  private:
    Type(string display, string comment) {
      m_Display = display;
      m_Comment = comment;
    }
  ...
  protected:
     ...
     */
  private:
    Field(string display, string comment) {
      m_Display = display;
      m_Comment = comment;
    }
...
protected:
  const static string MISSING_ID = "missing_id";
  ...
  Vector m_Additional = new Vector();
...
public:
   ...
   * @see Type
   */
  TechnicalInformation(Type type) {
    this(type, "");
  }
   ...
   * @see Type
   */
  TechnicalInformation(Type type, string id) {
    m_Type = type;
    m_ID   = id;
  }
...
public:
  ...
  void add(TechnicalInformation value) {
    if (value == this)
      throw IllegalArgumentException("Can't add object to itself!");
   ...
   */
  TechnicalInformation add(Type type) {
...
static void main(String[] args) throws Exception {
  ...
  if (args.length != 0) {
    ...
    if (tmpStr.length() != 0) {
      ...
      throw IllegalArgumentException("A classname has to be provided with the -W option!");
    }
